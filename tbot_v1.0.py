...
from openai import OpenAI  # Modified: Import OpenAI client
...
"""
    ...
    def _get_ai_analysis(self, context: str) -> str:
        ...
        try:
            response = client.chat.completions.create( # Modified: Use client.chat.completions.create
                model="gpt-4",
                messages=[
                    {
                        "role": "system", 
...
def create_main_keyboard():
    ...
    # Modified: Add new buttons for AI chat and PDF upload
    keyboard.add(
...
# Modified: Add new handlers for AI chat and PDF upload
...
def handle_ai_chat_request(message):
    ...
    user_passwords[user_id] = "awaiting_ai_question" # Use user_passwords to store state
...
def handle_ai_chat_input(message):
    """معالج رد المستخدم لسؤال الذكاء الاصطناعي"""
    user_id = message.from_user.id
    ...
    try:
        ...
        logger.error(f"خطأ أثناء التحدث إلى الذكاء الاصطناعي: {e}")
        bot.reply_to(message, f"❌ حدث خطأ أثناء التحدث إلى الذكاء الاصطناعي: {e}")

    user_passwords.pop(user_id, None) # Clear the state

@bot.message_handler(func=lambda message: message.text == "📚 رفع كتب PDF")
def handle_pdf_upload_request(message):
    """معالج زر 'رفع كتب PDF'"""
    user_id = message.from_user.id
    if not is_authenticated(user_id):
        bot.reply_to(message, "🔐 يرجى المصادقة أولاً")
        return
    bot.reply_to(message, "📚 يرجى إرسال ملف PDF الآن. سيتم تدريبه لاحقًا.")
...
def handle_uploaded_pdf(message):
    ...
    if document.mime_type == "application/pdf":
        ...
        # Ensure pdf_storage directory exists
        if not os.path.exists("pdf_storage"):
            os.makedirs("pdf_storage")

        save_path = f"pdf_storage/{document.file_name}"
        with open(save_path, "wb") as f:
            f.write(downloaded_file)

        bot.reply_to(message, f"✅ تم رفع الكتاب بنجاح: {document.file_name}\n📌 سيتم تدريبه لاحقًا.")
        logger.info(f"User {user_id} uploaded PDF: {document.file_name}")
    else:
        bot.reply_to(message, "❌ الملف المرفوع ليس من نوع PDF.")
...
def main():
    ...
    try:
        ...
        # Create pdf_storage directory if it doesn't exist
        if not os.path.exists("pdf_storage"):
            os.makedirs("pdf_storage")
